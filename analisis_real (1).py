...
# =======================================================================
# 5. REVISIÓN CON PINZAS — ANÁLISIS ESTACIONAL Y CASO EXTREMO (no promedio)
# =======================================================================
print("\n" + "="*70)
print("ANÁLISIS DE RIGOR: estacionalidad y caso más severo")
print("="*70)

# --- 5.1 Reconstruir fecha real de cada hora para poder filtrar por mes ---
fechas = []
with open("datos_kayra.csv") as f:
    lines = f.readlines()
start_idx = 0
...
for line in lines[start_idx:]:
    ...
    if len(parts) < 5:
        continue
    year, mo, dy, hr, t2m = parts
    fechas.append((int(year), int(mo), int(dy)))

meses = np.array([f[1] for f in fechas])

# --- 5.2 Ajuste de Fourier SOLO en época seca (mayo-agosto), donde la helada es más fuerte ---
mask_seca = np.isin(meses, [5, 6, 7, 8])
temp_seca = temperaturas[mask_seca]
t_seca = np.arange(len(temp_seca))

T_media_seca, a_n_seca, b_n_seca, omega_seca = ajustar_fourier(t_seca, temp_seca, n_armonicos=5)
amplitud_seca = np.sqrt(a_n_seca[0]**2 + b_n_seca[0]**2)

print(f"\nÉpoca seca (may-ago), {mask_seca.sum()} horas analizadas:")
print(f"  Temperatura media: {T_media_seca:.2f} °C")
print(f"  Amplitud diaria (época seca): {amplitud_seca:.2f} °C  <-- comparar con {amplitud_1:.2f} °C del promedio anual")

# --- 5.3 Gradiente y esfuerzo térmico MÁXIMO real (no un día arbitrario) ---
# recorremos cada día completo del registro y calculamos su gradiente superficie-20cm
n_dias = len(temperaturas) // 24
gradientes_diarios = []
for d in range(n_dias):
    t_dia = t_horas[d*24:(d+1)*24]
    T_sup = temperatura_en_profundidad(0.0, t_dia, T_media, a_n, b_n, omega, alpha_concreto)
    T_20 = temperatura_en_profundidad(0.20, t_dia, T_media, a_n, b_n, omega, alpha_concreto)
    gradientes_diarios.append((T_sup - T_20).max())

gradientes_diarios = np.array(gradientes_diarios)
dia_max = np.argmax(gradientes_diarios)
gradiente_max_real = gradientes_diarios[dia_max]
gradiente_promedio = gradientes_diarios.mean()

print(f"\nGradiente térmico (superficie vs 20cm) sobre los {n_dias} días del registro:")
print(f"  Promedio: {gradiente_promedio:.2f} °C")
print(f"  MÁXIMO real: {gradiente_max_real:.2f} °C (día #{dia_max} del registro)")

sigma_max = esfuerzo_termico_bradbury(gradiente_max_real, E=24600, alpha_c=1.3e-5, nu=0.15)
sigma_promedio = esfuerzo_termico_bradbury(gradiente_promedio, E=24600, alpha_c=1.3e-5, nu=0.15)
print(f"\nEsfuerzo térmico PROMEDIO: {sigma_promedio:.3f} MPa")
print(f"Esfuerzo térmico MÁXIMO (caso más severo real): {sigma_max:.3f} MPa")
